...
class SLProxy
{
  private:
    ...
    inline static PFun_slReflexSetCameraData* _slReflexSetCameraData = nullptr;
    inline static PFun_slReflexGetPredictedCameraData* _slReflexGetPredictedCameraData = nullptr;
  ...
  public:
    ...
    static bool InitSL()
    {
        ...
        if (!std::filesystem::exists(slInterposerOriginal))
        {
            LOG_ERROR(L"Missing Streamline interposer: {}", slInterposerOriginal.wstring());
            return false;
        }

        std::error_code ec;
        bool refreshRenamedCopy = !std::filesystem::exists(slInterposerRenamed, ec);
        if (!refreshRenamedCopy && !ec)
        {
            const auto originalSize = std::filesystem::file_size(slInterposerOriginal, ec);
            const auto renamedSize = !ec ? std::filesystem::file_size(slInterposerRenamed, ec) : 0;
            if (!ec && originalSize != renamedSize)
                refreshRenamedCopy = true;

            const auto originalWriteTime = std::filesystem::last_write_time(slInterposerOriginal, ec);
            const auto renamedWriteTime = !ec ? std::filesystem::last_write_time(slInterposerRenamed, ec) : originalWriteTime;
            if (!ec && originalWriteTime > renamedWriteTime)
                refreshRenamedCopy = true;
        }
        if (ec)
            refreshRenamedCopy = true;

        // Keep the renamed copy in sync. SLProxy loads this renamed file to avoid
        // OptiScaler's own name-based Streamline hook paths.
        if (refreshRenamedCopy)
        {
            ...
            LOG_INFO("Refreshed sl.interposer_output.dll from sl.interposer.dll");
        }
    ...
    static bool ResolveReflexFunctions()
    {
        ...
        // Resolve Frame Warp / Reflex 2 camera data APIs. These are optional
        // and can exist in the Reflex plugin even when full Frame Warp is gated.
        funcPtr = nullptr;
        result = _slGetFeatureFunction(sl::kFeatureReflex, "slReflexSetCameraData", funcPtr);
        if (result == sl::Result::eOk && funcPtr != nullptr)
        {
            _slReflexSetCameraData = (PFun_slReflexSetCameraData*) funcPtr;
            LOG_INFO("slReflexSetCameraData resolved");
        }
        else
        {
            LOG_INFO("slReflexSetCameraData not available: result={} ptr={}", (int) result, funcPtr != nullptr);
        }

        funcPtr = nullptr;
        result = _slGetFeatureFunction(sl::kFeatureReflex, "slReflexGetPredictedCameraData", funcPtr);
        if (result == sl::Result::eOk && funcPtr != nullptr)
        {
            _slReflexGetPredictedCameraData = (PFun_slReflexGetPredictedCameraData*) funcPtr;
            LOG_INFO("slReflexGetPredictedCameraData resolved");
        }
        else
        {
            LOG_INFO("slReflexGetPredictedCameraData not available: result={} ptr={}", (int) result, funcPtr != nullptr);
        }

        LOG_INFO("Reflex camera probe APIs: setCameraData={} getPredictedCameraData={}",
    ...
    static PFun_slReflexSetCameraData* ReflexSetCameraData() { return _slReflexSetCameraData; }
    static PFun_slReflexGetPredictedCameraData* ReflexGetPredictedCameraData() { return _slReflexGetPredictedCameraData; }
};
